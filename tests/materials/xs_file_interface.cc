...
// Environment variable guard to check the env var getter actually throws
// properly Windows makes this complicated. Like always.
...
TEST_F(MaterialsXSFileInterface, LoadEvaluationEnergies) {
  ...
  EXPECT_THROW(interface_->LoadEvaluationEnergies("293K", energies),
               std::runtime_error);
  // invalid formatted dataset (char instead of float in this case)
  XSFileInterface bad_file("BadFakeU235");
  EXPECT_THROW(bad_file.LoadEvaluationEnergies("294K", energies),
               std::runtime_error);
}
...
TEST_F(MaterialsXSFileInterface, Get1DDatasetSize) {
  ...
  EXPECT_THROW(interface_->Get1DDatasetSize("not_real_path"),
               std::runtime_error);
}
...
TEST_F(MaterialsXSFileInterface, Load1DXSDataset) {
  ...
  std::vector<float> xs;
  ASSERT_NO_THROW(
      interface_->Load1DXSDataset("002", "294K", xs, energies.size()));
  EXPECT_FALSE(xs.empty());
  EXPECT_EQ(xs.size(), energies.size());

  // wrong target size
  EXPECT_THROW(interface_->Load1DXSDataset("002", "294K", xs, 0),
               std::runtime_error);
  // invalid dataset path
  EXPECT_THROW(interface_->Load1DXSDataset("not", "real", xs, energies.size()),
               std::runtime_error);
  // invalid formatted dataset (char instead of float in this case)
  XSFileInterface bad_file("BadFakeU235");
  EXPECT_THROW(bad_file.Load1DXSDataset("002", "294K", xs, energies.size()),
               std::runtime_error);
}
...
TEST_F(MaterialsXSFileInterface, LeftPadLoad1DXSDataset) {
  ...
  ASSERT_NO_THROW(
      interface_->Load1DXSDataset("002", "294K", xs_real, energies.size()));

  std::vector<float> xs_padded;
  ASSERT_NO_THROW(interface_->LeftPadLoad1DXSDataset("002", "294K", xs_padded,
                                                     energies.size() + 1));
  ...
  // wrong target size
  EXPECT_THROW(interface_->LeftPadLoad1DXSDataset("002", "294K", xs_padded,
                                                  energies.size() - 1),
               std::runtime_error);
  // invalid dataset path
  EXPECT_THROW(interface_->LeftPadLoad1DXSDataset("not", "real", xs_padded,
                                                  energies.size() + 1),
               std::runtime_error);
  // invalid formatted dataset (char instead of float in this case)
  XSFileInterface bad_file("BadFakeU235");
  EXPECT_THROW(bad_file.LeftPadLoad1DXSDataset("002", "294K", xs_padded,
                                               energies.size() + 1),
               std::runtime_error);
}
