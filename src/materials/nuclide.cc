...
void Nuclide::LoadFromFile() {
  ...
  xs_file.Load1DXSDataset("002", temperature_, xs_map_[MT::ELASTIC], evaluation_energies_.size());
  xs_file.LeftPadLoad1DXSDataset("004", temperature_, xs_map_[MT::INELASTIC], evaluation_energies_.size());
  xs_file.Load1DXSDataset("018", temperature_, xs_map_[MT::FISSION], evaluation_energies_.size());
  xs_file.Load1DXSDataset("102", temperature_, xs_map_[MT::CAPTURE], evaluation_energies_.size());
...
double Nuclide::GetTotalXS(const size_t energy_index, const double energy) const {
...
double Nuclide::GetXSFromMT(MT mt, const size_t energy_index, const double energy) const {

  const float* xs = xs_map_.at(mt).data();
  if (energy <= evaluation_energies_.front()) return xs[0];
  if (energy >= evaluation_energies_.back()) return xs[evaluation_energies_.size() - 1];
